...
### Steps 2–6 — Run All Sources in Parallel + Merge + Dedup (ONE command)
```bash
node lib/run-search.mjs --specialty "{specialty}" --state "{state_code}" --city "{city}" [--job-id {id}]
```
This single command fires all 4 scrapers **concurrently** (NPI + LinkedIn + Doximity + Healthgrades),
waits for all to finish, merges + deduplicates, saves candidates to SQLite, and emits **slim JSON**
(~65% fewer tokens than raw output). Parse the JSON output — top-level fields:
- `search_id` — ID of this search run in SQLite
- `job_id` — job this search was for
- `total` — unique candidates after dedup
- `by_source` — counts per source `{npi, linkedin, doximity, healthgrades}`
- `wall_time_s` — total elapsed seconds (all parallel)
- `cache_hits` — candidates with scores from last 30 days (skip re-scoring these)
- `candidates[]` — slim records: `{id, n, cr, loc, ph, src, urls, cached?}`
  - `id` = SQLite candidate ID, `n` = name, `cr` = credential, `loc` = "City, ST"
  - `ph` = "yes"/"no" for phone, `src` = source array, `urls` = profile link count
  - `cached: true` = already scored in last 30 days — skip scoring, read from DB

### Step 7 — Score Each Candidate (YOU do this — no extra tools needed)
Read the slim candidates array from run-search output. For each candidate **without** `cached: true`:
...
- **Data completeness** (0-1 pt): Has phone ("yes") AND at least 1 URL

Also write:
- `match_reason` — 1 sentence why they're a strong fit
- `outreach_line` — 1 personalized opener for a cold message

**For background/async scoring** (large batches, don't want to wait in chat):
```bash
node lib/batch-score.mjs score --job-id {id} --search-id {id} [--concurrency 5]
```
Spawns N parallel `claude -p` workers (default 5), each scoring one candidate.
No API key needed — uses Claude Code directly. Results written to SQLite automatically.
Check progress: `node lib/batch-score.mjs status --job-id {id}`
